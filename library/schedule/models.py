...
class Teacher(models.Model):
    ...
    email = models.EmailField(unique=True, verbose_name="Email")  # Уникальное поле
    ...
    class Meta:
        ...
        # Ограничение: нельзя удалить преподавателя, если у него есть активные курсы (логика будет в коде)
        # Но на уровне БД можно добавить constraint, если нужно
        ordering = ['last_name', 'first_name']


class TeacherInfo(models.Model):
    """Расширенная информация о преподавателе (связь 1:1)"""
    # Связь 1:1 с преподавателем. on_delete=CASCADE — при удалении учителя удалится и его профиль.
...
class Course(models.Model):
    ...
    # on_delete=SET_NULL — если преподавателя удалят, у курса останется информация, но teacher станет NULL.
    ...
    class Meta:
        ...
        # Ограничение уникальности: нельзя создать два одинаковых курса у одного преподавателя
...
class Student(models.Model):
    ...
    # Связь N:N с курсами. related_name='students' позволит из Course получить список студентов.
    # Указываем through, если нужны дополнительные поля в enrollment (например, дата записи).
